...
class SpeedProvider(private val context: Context) {
    ...
    private var lastReading: Reading = Reading(0f, Source.UNKNOWN, 0f, 0L)
    ...
    fun getCurrent(): Reading {
        ...
        val selected = when {
            ble != null && now - ble.timestampMs <= 2000L -> Reading(ble.speedMps, Source.BLE, 0.95f, ble.timestampMs)
            gps != null && now - gps.timestampMs <= 2000L -> Reading(gps.speedMps, Source.GPS, 0.85f, gps.timestampMs)
            gps != null && now - gps.timestampMs <= 4000L -> Reading(gps.speedMps, Source.GPS, 0.55f, gps.timestampMs)
            else -> Reading(0f, Source.UNKNOWN, 0f, now)
        }

        val held = if (selected.source == Source.UNKNOWN && now - lastReading.timestampMs <= 1500L) {
